...
def greetings_time():
    ...
    if current_time <= time(hour=12):
        ...
        utils_logger = logging.getLogger("Приветствие - Доброе утро")
    elif current_time <= time(hour=18):
        greeting = "Добрый день"
        utils_logger = logging.getLogger("Приветствие - Добрый день")
    elif current_time <= time(hour=23):
        greeting = "Добрый вечер"
        utils_logger = logging.getLogger("Приветствие - Добрый вечер")
    else:
        greeting = "Доброй ночи"
        utils_logger = logging.getLogger("Приветствие - Доброй ночи")
...
def get_card_data(cards):
    ...
    for card in cards:
        ...
        utils_logger = logging.getLogger("Последние 4 цифры карты")
        total_spent = sum(card["transactions"])
        utils_logger = logging.getLogger("Общая сумма расходов")
        cashback = total_spent // 100
        utils_logger = logging.getLogger("Кэшбек")
        card_info = {"last_four_digits": last_four_digits, "total_spent": total_spent, "cashback": cashback}
    response.append(card_info)
    utils_logger = logging.getLogger("Информация по карте")
...
def get_top_transactions(transactions):
    ...
    utils_logger = logging.getLogger("Вывод Топ-5 транзакций")
    return json.dumps(response)
...
def get_currency_rates(api_key_1):
    ...
    if response.status_code == 200:
        utils_logger = logging.getLogger("Вывод курса валют")
        return response.json()
    else:
        utils_logger = logging.getLogger("Ошибка вывода курса валют")
...
def get_sp500_stock_prices(api_key_2):
    ...
    if response.status_code == 200:
        data = response.json()
        utils_logger = logging.getLogger("Вывод информации о стоимости акций S&P500")
        return json.dumps(data, indent=4)
    else:
        utils_logger = logging.getLogger("Ошибка вывода о стоимости")
